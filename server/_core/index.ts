...
// Flair Routes
app.post('/api/flairs', authenticateToken, isAdmin, async (req, res) => {
  const { name, icon } = req.body;
  const flair = await prisma.flair.create({ data: { name, icon } });
  res.json(flair);
});

app.get('/api/flairs', async (req, res) => {
  const flairs = await prisma.flair.findMany();
  res.json(flairs);
});

// CustomEmoji Routes
app.post('/api/emojis', authenticateToken, isAdmin, async (req, res) => {
...
// Shortcut Routes
app.post('/api/shortcuts', authenticateToken, isAdmin, async (req, res) => {
...
app.post('/api/users/:id/flair', authenticateToken, isAdmin, async (req, res) => {
  const { id } = req.params;
